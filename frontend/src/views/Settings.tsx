...
export default function System(_props: Props) {
  ...
  const statusRows: StatusRow[] = [
    ...
    },
    {
      ...
      value: status
        ? `${status.entry_count} Einträge`
        : statusError
          ? "Nicht erreichbar"
          : "—",
      sub: status
        ? `${fmtBytes(status.db_size_bytes)} · ${status.chunk_count} Chunks`
        : undefined,
    },
    {
      ...
      value: !status
        ? "—"
        : status.unembedded_chunks === 0
          ? "Alles eingebettet"
          : `${status.unembedded_chunks} ausstehend`,
      sub:
        status && status.unembedded_chunks > 0
          ? `von ${status.chunk_count} Chunks`
          : undefined,
    },
    {
      label: "Meilisearch",
      state: !status
        ? "warn"
        : status.meilisearch_stats
          ? status.meilisearch_stats.is_indexing
            ? "warn"
            : "ok"
          : "err",
      value: !status
        ? "—"
        : status.meilisearch_stats
          ? `${status.meilisearch_stats.number_of_documents} indiziert${status.meilisearch_stats.is_indexing ? " (arbeitet…)" : ""}`
          : "Nicht erreichbar",
  ...
  const configGroups = [
    {
      ...
      rows: [
        { label: "Zeitzone", value: status?.tz ?? "—", env: "TZ" },
        {
          ...
          value: status?.db_path_str ?? "—",
          env: "DB_PATH",
        },
        {
          label: "Upload-Pfad",
          value: status?.upload_path ?? "—",
          env: "UPLOAD_PATH",
        },
        {
          label: "Meilisearch URL",
          value: status?.meilisearch_url ?? "—",
          env: "MEILISEARCH_URL",
        },
        {
          label: "CORS Origins",
          value: status?.cors_origins || "Standard",
          env: "CORS_ORIGINS",
        },
        {
          label: "HuggingFace SSL-Prüfung",
          value: status
            ? status.ssl_verify
              ? "Aktiv"
              : "Deaktiviert (Proxy Workaround)"
            : "—",
          env: "SSL_VERIFY",
        },
        {
          label: "Demo-Modus",
          value: status ? (status.demo ? "Aktiv" : "Inaktiv") : "—",
          env: "DEMO",
        },
      ],
    },
    {
      ...
      rows: [
        ...
        {
          label: "Schriftarten",
          value: settings.branding_font || "—",
          env: "FONT_STACK",
        },
        {
          label: "Eigenes Logo",
          value: settings.branding_logo_b64 ? "Gesetzt" : "Nicht gesetzt",
          env: "BRANDING_LOGO_FILE",
        },
        {
          label: "Custom CSS",
          value: settings.branding_custom_css ? "Gesetzt" : "Nicht gesetzt",
          env: "CUSTOM_CSS / CUSTOM_CSS_FILE",
        },
      ],
    },
    {
      ...
      rows: [
        {
          ...
          value: settings.search_threshold?.toFixed(2) ?? "—",
          env: "SEARCH_THRESHOLD",
        },
        {
          label: "Duplikat-Schwelle",
          value: settings.dupe_threshold?.toFixed(2) ?? "—",
          env: "DUPE_THRESHOLD",
        },
        {
          label: "Top-K Ergebnisse",
          value: String(settings.top_k ?? "—"),
          env: "TOP_K",
        },
      ],
    },
    {
      ...
      rows: [
        {
          label: "Chunk-Größe",
          value: `${settings.chunk_size} Zeichen`,
          env: "CHUNK_SIZE",
        },
        {
          label: "Chunk-Überlappung",
          value: `${settings.chunk_overlap} Zeichen`,
          env: "CHUNK_OVERLAP",
        },
      ],
    },
    {
      ...
      rows: [
        ...
        },
        {
          ...
          value: String(settings.agent_max_turns ?? "—"),
          env: "AGENT_MAX_TURNS",
        },
      ],
    },
      ...
      <div className="page-head">
        <div>
          ...
          <p className="page-sub">
            Status und Konfiguration. Werte werden über Umgebungsvariablen
            gesetzt.
          </p>
        </div>
      </div>
