...
function FAQWriteForm({ toast, settings }: Props) {
  ...
  useEffect(() => {
    ...
    suggestTags(text)
      .then(setSuggestedTagList)
      .catch(() => {});
  }, [debouncedQ, debouncedA]);

  const handleSave = async () => {
    if (!question.trim()) {
      toast("Frage darf nicht leer sein.", "error");
      return;
    }
    if (!answer.trim()) {
      toast("Antwort darf nicht leer sein.", "error");
      return;
    }
    setSaving(true);
    try {
      const saved = await createQA({
        title: title.trim(),
  ...
  const hasContent =
    debouncedQ.trim().length > 5 || debouncedA.trim().length > 5;
    ...
    <div className="editor">
      <div className="editor-main">
        <div className="field">
          <label>
            ...
            <span className="field-hint">
              Kurzform – leer lassen für Fallback auf die Frage
            </span>
          </label>
          <input
        ...
        <DupeWarning
          ...
          hasContent={hasContent && hasChecked}
        />
      ...
      <div className="editor-side">
        <div className="aside-card">
          <h4>Tipps</h4>
          <p>
            Verwende eine klare, konkrete Frage. Die Suche findet auch ähnlich
...
function DOKWriteForm({ toast }: Props) {
  ...
  const handleSave = async () => {
    if (!content.trim()) {
      toast("Inhalt darf nicht leer sein.", "error");
      return;
    }
    setSaving(true);
    try {
    ...
    <div className="editor">
      <div className="editor-main">
        <div className="field">
          <label>
            Titel{" "}
            <span className="field-hint">
              Leer lassen für Fallback auf erste Zeile
            </span>
          </label>
          <input
            className="txt title-input"
      ...
      <div className="editor-side">
        <div className="aside-card">
          <h4>Hinweis</h4>
          <p>
            Der Inhalt wird nach dem Speichern in Abschnitte aufgeteilt und
            eingebettet.
          </p>
          <p>Tags helfen bei der Filterung — z. B. IT, HR, Prozesse.</p>
        </div>
...
function UnifiedImportSection({ toast }: { toast: Props["toast"] }) {
    ...
    listImports(1)
      .then((resp) => {
        ...
        const relevant = resp.items.filter((item) => {
          const done =
            item.chunk_count > 0 && item.embedded_count >= item.chunk_count;
          const recent = item.created_at
            ? now - new Date(item.created_at).getTime() < FIVE_MIN
            : false;
          return !done || recent;
        });
  ...
  const handleDocFile = async (file: File) => {
    ...
    try {
      ...
      setUploads((prev) =>
        prev.map((u) =>
          u.file === displayPath && u.entry_id === null
            ? {
                ...u,
      ...
      toast(
        `"${result.title}" importiert (${result.chunk_count} Abschnitte)`,
        "success",
      );
    } catch (e) {
      setUploads((prev) =>
  ...
  const handleFiles = async (files: File[] | FileList) => {
    ...
    const qaFiles = fileArray.filter((f) =>
      ["csv", "json", "yaml", "yml"].includes(
        f.name.split(".").pop()?.toLowerCase() ?? "",
      ),
    );
    const docFiles = fileArray.filter((f) =>
      ["md", "pdf", "docx", "doc"].includes(
        f.name.split(".").pop()?.toLowerCase() ?? "",
      ),
    );
    const unsupported = fileArray.filter(
      (f) =>
        !["csv", "json", "yaml", "yml", "md", "pdf", "docx", "doc"].includes(
          f.name.split(".").pop()?.toLowerCase() ?? "",
        ),
    );

    for (const f of unsupported)
      toast(`Format nicht unterstützt: .${f.name.split(".").pop()}`, "error");
  ...
  const handleTagChange = async (entry_id: number, newTags: string[]) => {
    setUploads((prev) =>
      prev.map((u) => (u.entry_id === entry_id ? { ...u, tags: newTags } : u)),
    );
  ...
  const setAction = (
    ...
    setRows((prev) =>
      prev.map((r, idx) =>
        idx === i ? { ...r, action, replace_id: replaceId ?? r.replace_id } : r,
      ),
    );
  ...
  const setRowTitle = (i: number, t: string) =>
    setRows((prev) =>
      prev.map((r, idx) => (idx === i ? { ...r, title: t } : r)),
    );
        ...
        if (
          ...
          getEntry(r.duplicates[0].id)
            .then((e) =>
              setRows((p) =>
                p.map((x, xi) =>
                  xi === i ? { ...x, dupeEntry: e, dupeLoading: false } : x,
                ),
              ),
            )
            .catch(() =>
              setRows((p) =>
                p.map((x, xi) =>
                  xi === i ? { ...x, dupeEntry: null, dupeLoading: false } : x,
                ),
              ),
            );
        }
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast(
        `${result.imported} importiert, ${result.replaced} ersetzt, ${result.skipped} übersprungen`,
        "success",
      );
      resetCsv();
    } catch (e) {
  ...
  return (
    <>
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 11, opacity: 0.6 }}>
              CSV / JSON / YAML → FAQ-Einträge &nbsp;·&nbsp; PDF / DOCX / MD →
              Dokument
            </p>
            <input
              ref={fileInputRef}
          ...
          <details
            className="import-templates-accordion"
            style={{
              padding: "1rem",
          ...
          >
            ...
            >
              Format-Beispiele für FAQ-Import anzeigen
            </summary>
            <div
              style={{
                display: "flex",
              ...
              <div>
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    <button
                      className="btn btn--ghost btn--sm"
                      style={{ width: 95, height: 26, padding: 0 }}
                      onClick={(e) => copyTemplate("json", e)}
                    >
                      {copiedFormat === "json" ? "✓ Kopiert" : "Kopieren"}
                    </button>
                    <button
                      className="btn btn--ghost btn--sm"
                      onClick={(e) => downloadTemplate("json", e)}
                    >
                      Herunterladen
                    </button>
                  </div>
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    <button
                      className="btn btn--ghost btn--sm"
                      style={{ width: 95, height: 26, padding: 0 }}
                      onClick={(e) => copyTemplate("yaml", e)}
                    >
                      {copiedFormat === "yaml" ? "✓ Kopiert" : "Kopieren"}
                    </button>
                    <button
                      className="btn btn--ghost btn--sm"
                      onClick={(e) => downloadTemplate("yaml", e)}
                    >
                      Herunterladen
                    </button>
                  </div>
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    <button
                      className="btn btn--ghost btn--sm"
                      style={{ width: 95, height: 26, padding: 0 }}
                      onClick={(e) => copyTemplate("csv", e)}
                    >
                      {copiedFormat === "csv" ? "✓ Kopiert" : "Kopieren"}
                    </button>
                    <button
                      className="btn btn--ghost btn--sm"
                      onClick={(e) => downloadTemplate("csv", e)}
                    >
                      Herunterladen
                    </button>
                  </div>
        ...
        <div className="qa-review">
          <div className="qa-review-bar">
            <div className="qa-review-stats">
              ...
              {rows.length > 0 && (
                <span>{parseComplete ? `${rows.length} Einträge` : ""}</span>
              )}
              {dupeCount > 0 && (
                <span className="qa-stat-warn">{dupeCount} Duplikate</span>
              )}
              {parseComplete && (
                <span className="qa-stat-new">{newCount} importieren</span>
              )}
              {replaceCount > 0 && (
                <span className="qa-stat-replace">{replaceCount} ersetzen</span>
              )}
              {skipCount > 0 && (
                <span className="qa-stat-skip">{skipCount} überspringen</span>
              )}
            </div>
            <div className="qa-review-actions">
              <button className="btn btn--ghost btn--sm" onClick={resetCsv}>
                Abbrechen
              </button>
              <button
          ...
          <div className="qa-split-layout">
            <div className="qa-split-sidebar">
              {rows.map((row, i) => (
                ...
                >
                  <div className="qa-split-item-q">
                    {row.question || "Leere Frage"}
                  </div>
                  <div className="qa-split-item-status">
                    {row.action === "skip"
                      ...
                      : row.action === "replace"
                        ? "Ersetzen"
                        : row.duplicates.length > 0
                          ? `${row.duplicates.length} Konflikte`
                          : "Neu"}
                  </div>
                </div>
              ))}
      ...
      {uploads.length > 0 && (
        <>
          <p className="section-h" style={{ marginTop: 20 }}>
            Aktuelle Importe
          </p>
          <div className="import-queue">
            {uploads.map((u, i) => {
              const pct =
                u.chunk_count > 0
                  ? Math.round((u.embedded_count / u.chunk_count) * 100)
                  : 0;
              return (
                    ...
                    >
                      <div style={{ flex: 1 }}>
                        <TagInput
                          tags={u.tags}
                          onChange={(newTags) =>
                            handleTagChange(u.entry_id!, newTags)
                          }
                          suggestions={allTags}
                        />
                      </div>
                      {u.done && (
                        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                          <button
                            className="btn btn--ghost btn--sm"
                            onClick={() => navigate(`/entries/${u.entry_id}`)}
                          >
                            Anzeigen
                          </button>
                          <button
                            className="btn btn--ghost btn--sm"
                            onClick={() => navigate(`/editor/${u.entry_id}`)}
                          >
                            Bearbeiten
                          </button>
                        </div>
  ...
  return (
    <main className="pb-main">
      <div className="page-head">
        <div>
          ...
          <p className="page-sub">
            FAQ-Einträge schreiben, Dokumente verfassen oder Dateien
            importieren.
          </p>
        </div>
      </div>
      ...
      <div className="import-tabs">
        <button
          className={`import-tab ${tab === "faq" ? "active" : ""}`}
          onClick={() => setTab("faq")}
        >
          FAQ schreiben
        </button>
        <button
          className={`import-tab ${tab === "dok" ? "active" : ""}`}
          onClick={() => setTab("dok")}
        >
          Dokument schreiben
        </button>
