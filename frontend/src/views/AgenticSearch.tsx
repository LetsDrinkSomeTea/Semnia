        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (payload.type === "results") {
              ...
              updateLastTurn((t) => {
                const existing = t.steps.findIndex(
                  (s) => s.id === payload.call_id,
                );
                if (existing === -1) {
                  return {
                    ...t,
                  ...
                  nextSteps[existing] = {
                    ...nextSteps[existing],
                    args: payload.args,
                    title: payload.title || nextSteps[existing].title,
                  };
              ...
              updateLastTurn((t) => ({
                ...t,
                summary: t.summary + payload.text,
              }));
    ...
    summarizeStream(
      ...
      (token) =>
        setTurns((prev) => {
          ...
          next[turnIdx] = {
            ...next[turnIdx],
            llmAnswer: (next[turnIdx].llmAnswer ?? "") + token,
          };
          return next;
        }),
      () =>
          ...
          {turns.length === 0 && !running && (
            ...
            >
              <p style={{ fontSize: "1.1rem" }}>
                Stelle eine Frage, und der Agent durchsucht autonom die
                Wissensdatenbank.
              </p>
            </div>
          )}
          ...
          {turns.map((turn, tIdx) => (
            <div key={tIdx} style={{ marginBottom: 32 }}>
              ...
              {turn.failureReason && (
                ...
                >
                  <strong>⚠ Agent hat die Suche abgebrochen:</strong>{" "}
                  {turn.failureReason}
                </div>
              )}
                          ...
                          return usedSources.length > 0 ? (
                            <div className="llm-sources">
                              {usedSources.map(({ s, n }) => (
                                <button
                                  key={s.id}
                                  className="llm-source-item"
                                  onClick={() => navigate(`/entries/${s.id}`)}
                                >
                                  <span className="llm-source-n">{n}</span>
                                  <span className="llm-source-title">
                                    {s.display_title}
                                  </span>
                                </button>
                              ))}
              ...
              <span>
                {(() => {
                  ...
                  if (!currentTurn || currentTurn.steps.length === 0)
                    return "Agent überlegt...";
                  const lastStep =
                    currentTurn.steps[currentTurn.steps.length - 1];
                  ...
                  try {
                    ...
                    if (typeof lastStep.args === "string") {
                      const qMatch = lastStep.args.match(
                        /"query"\s*:\s*"([^"]+)/,
                      );
                      if (qMatch) args.query = qMatch[1];
                      const idMatch = lastStep.args.match(
                        /"entry_id"\s*:\s*(\d+)/,
                      );
                  ...
                  if (lastStep.tool === "search_database") {
                    ...
                    return (
                      <>
                        Lese vollständiges Dokument "
                        <i>{lastStep.title || args.entry_id || ""}</i>"
                      </>
                    );
                    ...
                    return (
                      <>
                        Markiere Dokument "
                        <i>{lastStep.title || args.entry_id || ""}</i>" als
                        relevant
                      </>
                    );
                  } else {
                    return (
                      <>
                        Führe Aktion aus: "<i>{lastStep.tool}</i>"
                      </>
                    );
                  }
                })()}
