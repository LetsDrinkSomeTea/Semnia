...
export default function QAEditor({ toast, settings }: Props) {
  ...
  useEffect(() => {
    ...
    getEntry(Number(id))
      ...
      .catch(() => {
        toast("Eintrag nicht gefunden", "error");
        navigate(-1);
      });
  }, [id]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (entryType === "document") {
        ...
        const saved = await updateDoc(Number(id), {
          title: title.trim(),
        ...
        const saved = await updateQA(Number(id), {
          title: title.trim(),
  ...
  const handleCancel = async () => {
    ...
    if (
      (title || dirty || tags.length) &&
      !(await ask("Änderungen verwerfen?"))
    )
      return;
    navigate(`/entries/${id}`);
  };
            ...
            <div className="field">
              <label>
                ...
                <span className="field-hint">
                  Kurzform – leer lassen für Fallback auf erste Zeile
                </span>
              </label>
              <input
