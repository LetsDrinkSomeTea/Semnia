...
export const search = (body: SearchRequest) =>
  req<{ items: SearchResult[]; has_more: boolean }>("/search", {
    method: "POST",
...
export const uploadFile = async (
  file: File,
): Promise<{ entry_id: number; title: string; chunk_count: number }> => {
...
export const confirmQAImport = (items: QABulkAction[]) =>
  req<{ imported: number; skipped: number; replaced: number }>(
    "/import/qa/confirm",
    {
      method: "POST",
...
export const getImportStatus = (entry_id: number) =>
  req<{ chunk_count: number; embedded_count: number; done: boolean }>(
    `/import/${entry_id}/status`,
  );
