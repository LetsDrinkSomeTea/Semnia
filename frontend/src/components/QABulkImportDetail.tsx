...
interface Props {
  ...
  setAction: (i: number, action: ReviewRow['action'], replaceId?: number) => void
}
    ...
    <div className="qa-detail-card">
      ...
      <div className={isConflict ? 'qa-detail-split' : ''}>
        ...
        <div className="qa-detail-col">
          ...
          <TagInput tags={row.tags} onChange={(t) => setTags(i, t)} suggestedTags={row.suggested_tags} />
        </div>
          ...
          <div className="qa-detail-col">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '500px', overflowY: 'auto', paddingRight: '8px' }}>
              {row.duplicates.map((dupe) => {
                const isSelected = row.action === 'replace' && row.replace_id === dupe.id
                return (
                  <div 
                    key={dupe.id}
                    ...
                    style={{ 
                      cursor: 'pointer', 
                      margin: 0, 
                      borderColor: isSelected ? 'var(--base--action)' : 'var(--primary--200)',
                      boxShadow: isSelected ? '0 0 0 1px var(--base--action)' : 'none',
                      opacity: (row.action === 'replace' && !isSelected) ? 0.5 : 1,
                      transition: 'all var(--t)'
                    }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px', marginBottom: '8px' }}>
                      <div className="qa-detail-readonly-q" style={{ margin: 0 }}>{dupe.question}</div>
                      <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--base--action)', flexShrink: 0 }}>{Math.round(dupe.score * 100)}% Match</div>
                    </div>
      ...
      <div className="qa-detail-actions">
        <div className="qa-dupe-action-row">
          <label className={row.action === 'import' ? 'active' : ''}>
            ...
            {isConflict ? 'Trotzdem als neu importieren' : 'Als neu importieren'}
          </label>
          ...
          {isConflict && (
            <label className={row.action === 'replace' ? 'active' : ''}>
              <input type="radio" name={`action-${i}`} value="replace" checked={row.action === 'replace'} onChange={() => setAction(i, 'replace', row.replace_id || row.duplicates[0]?.id)} />
