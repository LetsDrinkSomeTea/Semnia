...
export default function TagInput({ tags, onChange, suggestions = [], suggestedTags = [] }: Props) {
  ...
  const ghostSuggestion = (() => {
    ...
    return filtered.find((s) => s.toLowerCase().startsWith(input.toLowerCase())) ?? null
  })()
  const ghostSuffix = ghostSuggestion ? ghostSuggestion.slice(input.length) : ''
    ...
    <div style={{ position: 'relative' }}>
      ...
      {showDropdown && (
        <div className="tag-dropdown">
          {!input && suggestedTags.filter((s) => !tags.includes(s)).length > 0 && (
